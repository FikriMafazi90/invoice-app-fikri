                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" id="INVOICE_ID" placeholder="INVOICE ID"
                                    name="INVOICE_ID" required="required" value="{{ $p->INVOICE_ID }}" readonly>
                            </div>
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" id="ID_MST_POPULATE_FROM" name="ID_MST_POPULATE_FROM">
                                    @foreach ($TBL_MST_POPULATE_FROM as $from)
                                        <option value="{{ $from->ID }}">{{ $from->NAME }} |
                                            {{ $from->DESCRIPTION }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input class="form-control" id="ISSUE_DATE" placeholder="ISSUE DATE" type="date"
                                    name="ISSUE_DATE" required="required" value="{{ $p->ISSUE_DATE }}">
                            </div>
                            <div class="form-group col-md-6">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input class="form-control" id="DUE_DATE" placeholder="DUE DATE" type="date"
                                    name="DUE_DATE" required="required" value="{{ $p->DUE_DATE }}">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label for="SUBJECT">SUBJECT</label>
                                <textarea class="form-control" id="SUBJECT" rows="3" required="required" name="SUBJECT">{{ $p->SUBJECT }}</textarea>
                            </div>
                        </div>
                            ...
                            @foreach ($DETAIL as $d)
                                <div style="margin-top:5px;margin-bottom:5px;width:100%;margin-left:5px;">
                                    <select id="idForm" name="idForm[]" style="width:50%;padding:0.2rem;">
                                        @foreach ($ITEM as $r)
                                            @if ($r->ID == $d->ID_MST_ITEM)
                                                <option selected value="{{ $r->ID }}">{{ $r->NAME }} |
                                                    {{ $r->DESCRIPTION }} |
                                                    {{ $r->PRICE }}
                                                </option>
                                            @else
                                    ...
                                    <input type="number" name="qtyForm[]" id="qtyForm" placeholder="QTY"
                                        required="required" style="width:20%;" value="{{ $d->QTY }}" />
